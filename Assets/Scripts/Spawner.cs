...
public class Spawner : MonoBehaviour
{
    ...
    public void SpawnObstacle()
    {
        ...
        var obsScript = obstacle.GetComponent<Obstacle>();
        if (obsScript != null)
        {
            obsScript.SetSpeed(obsScript.movementSpeed * currentSpeedMultiplier);
        }
    ...
    public void ModifyObstacleSpeed(float multiplier)
{
    ...
    // Update the speed of all existing objects
    foreach (Transform child in transform)
    {
        var obstacle = child.GetComponent<Obstacle>();
        if (obstacle != null)
        {
            obstacle.SetSpeed(obstacle.movementSpeed * multiplier);  // Applying a new speed multiplier
        }
    }
}
