...
public class Player : MonoBehaviour
{
    ...
    private IEnumerator TemporarilyChangeSprite(Sprite newSprite, float duration)
    {
        mSpriteRenderer.sprite = newSprite; 
    ...
    void Update()
    {
        ...
        if (verticalMovement != 0.0 && !mSwitchedGravity)
        {
            ...
            StartCoroutine(TemporarilyChangeSprite(spritePog, 1.0f));
    ...
    {   
        ...
        if (hitObstacle)
        { // If we collide with any obstacle on ground-> end the game.
            if(!onGround){
                mGameManager.AddScore(5);
                StartCoroutine(TemporarilyChangeSprite(spriteHappy, 1.0f));
